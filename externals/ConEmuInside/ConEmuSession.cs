...
namespace ConEmu.Inside
{
	...
	public class ConEmuSession
	{
		...
		private AnsiLog Init_AnsiLog([NotNull] ConEmuStartInfo startinfo)
		{
			...
			// On the home thread, as the WinForms timer of the original did (Git Extensions: without WinForms).
			SynchronizationContext homeContext = SynchronizationContext.Current;
			...
			var timer = new System.Threading.Timer(_ => { if (homeContext != null) homeContext.Post(Pump, null); else Pump(null); }, null, TimeSpan.FromSeconds(.1), TimeSpan.FromSeconds(.1));
			_lifetime.Add(() => { isDisposed = true; timer.Dispose(); });
