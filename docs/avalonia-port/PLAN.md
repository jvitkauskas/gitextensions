...
| 7: Plugin API v2 and switching the host | **In progress.** The Avalonia main window (`BrowseWindow`, with `GE_AVALONIA=all,FormBrowse`): the revision grid with the commit and diff tabs, the main menus and toolbar. The grid has its context menu. Next: the left panel (`RepoObjectsTree`), the dashboard and the file tree. |
| 8+ | Not started. |
