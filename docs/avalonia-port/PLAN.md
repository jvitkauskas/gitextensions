...
| 3: Text/diff viewer | **In progress.** Step 1 done: `TextEditorView` on AvaloniaEdit (MIT, added to `Product.wxs`) in editing mode, with `FormEditor` (git's editor), `FormGitAttributes` and `FormMailMap` on it. `DialogViewModel.CanClose` lets view models keep a dialog open (e.g. unsaved changes). Next: the viewer mode and the diff highlighting (separating it from ICSharpCode, see the phase), then `FormSparseWorkingCopy`, `FormViewPatch` and the diff dialogs. |
