...
| 3: Text/diff viewer | Not started. |
| 4: Revision grid | **In progress.** Step 1 done: the graph layout (`RevisionGraph`, rows, segments, lanes) is compiled by the UI-neutral `GitUI.RevisionGraph`, in place (see finding 30). Next: an Avalonia graph renderer and revision list, then `FormChooseCommit`. |
| 5+ | Not started. |
