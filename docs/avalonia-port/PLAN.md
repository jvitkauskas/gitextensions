...
| 7: Plugin API v2 and switching the host | **In progress.** The forms of the plugins (and the repository hosting dialogs of the GitHub plugin) are ported on the current plugin API. The Avalonia main window (`BrowseWindow`, with `GE_AVALONIA=all,FormBrowse`): the revision grid with the commit and diff tabs, the main menus and toolbar. The grid has its context menu; the file tree tab, the filter toolbar, the dashboard and switching repositories in the window are ported. The console and GPG tabs and the left panel (`RepoObjectsTree`) are ported. Next: the build report tab (with the build status of the grid), the plugins and scripts menus, the status bar. |
| 8+ | Not started. |
