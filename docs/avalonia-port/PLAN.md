...
| 2: Lightweight dialogs | **Done.** The progress dialog (`FormStatus`/`FormProcess`) covers the ~70 `FormProcess.ShowDialog`/`ReadDialog` and `FormStatus.ShowErrorDialog` call sites. Batch 1 adds command line help, add files, donate, contributors, reset changes, delete tag, init, go to line, and the script input and file prompts. Batch 2 adds the SSH key prompt, build server credentials, branch multi-selection, language selection, encodings, add submodule, clean working directory, submodule conflict, create worktree and open repository. Batch 3a adds delete branch, delete remote branch and merge branch, with ports of the `BranchComboBox` and `HelpImageDisplayUserControl` controls. Batch 3b adds cherry-pick, revert, reset current branch, reset another branch and archive, with a port of `CommitSummaryUserControl`. Batch 3c adds create branch and create tag, with a port of `CommitPickerSmallControl`. Batch 4 adds checkout revision, compare to branch, bisect, go to commit, dashboard category name and add to .gitignore. Batch 5 adds checkout branch. Batch 6 adds clone. Batch 7 adds the revision filter. Batch 8 adds the HOME directory, check for updates, worktree and submodule lists, and the modeless help text window of the scripts settings. Batch 9 adds the reflog and the recent repositories settings. Batch 10 adds the remotes dialog. The last batch adds the git command log, the git grep prompt, the quick item selector of the grid, `FormLog` and the search of the editors (`FindAndReplaceForm`). `FormRemoteProcess` is ported with phase 5. See the [ledger](ledger.md). |
| 3: Text/diff viewer | **In progress.** Step 1 done: `TextEditorView` on AvaloniaEdit (MIT, added to `Product.wxs`) in editing mode, with `FormEditor` (git's editor), `FormGitAttributes` and `FormMailMap` on it. `DialogViewModel.CanClose` lets view models keep a dialog open (e.g. unsaved changes). Step 2 done: a read-only diff mode (`TextEditorViewModel.LoadDiff`) with the two-column line-number margin and the added/removed/header backgrounds of a patch without git's colors (`DiffLinesAnalyzer`, a port of `DiffLineNumAnalyzer`), with `FormViewPatch` on it. `FormSparseWorkingCopy`, `FormGitIgnore` and `FormChangeLog` are on the editing mode; `FormVerify` (with `FormEdit`) previews lost objects in both modes. Step 3 done: the identical parts of matching removed and added lines are dimmed (`InlineDiffAnalyzer`, a port of `LinesMatcher` and `AddInlineDifferenceMarkers`), with a parity test against the WinForms viewer. Step 4 done: git's colored output (`AnsiEscapeParser`, a port of `AnsiEscapeUtilities`, with moved lines), also checked against the WinForms viewer. Step 5 done: the git word diff, difftastic, range diff, grep, combined and fixed diff appearances, the syntax highlighting of diffs, and the context menu and toolbar of `FileViewer` (see the ledger for the few items left). The blame margin is done, with the blame of phase 5. |
| 4: Revision grid | **In progress.** Step 1 done: the graph layout (`RevisionGraph`, rows, segments, lanes) is compiled by the UI-neutral `GitUI.RevisionGraph`, in place (see finding 30). Step 2 done: the Avalonia revision grid (`RevisionGridView`, graph drawn by `RevisionGraphRenderer`), and `FormChooseCommit` on it. Step 3 done: artificial commits, quick search and multi-selection; `FormFormatPatch`. Step 4 done: the grid context menu (the actions, copy, navigate, compare and view submenus, scripts) with the back/forward and parent/child navigation and the column settings. `FormLog` and `FormFileHistory` are ported. |
| 5: FileStatusList, commit editor, CommitInfo, big dialogs | **Done.** Step 1 done: the file status list (`FileStatusListViewModel`, `FileStatusListView`): the tree of one or more diffs with the status icons, multiple selection, the filter and the sortings, built by a port of `GetNodes` and `StatusSorter` checked against the WinForms list. Step 2 done: the file viewer (`FileViewerViewModel`, `FileViewerView`, a port of `ViewChangesAsync` in `FileViewerHost`) with texts, diffs with git's colors and images, and `FormDiff` on the list and the viewer. Step 3 done: the menu of the list for any diff (difftool, open, edit, save, copy paths, show in folder, history, blame, reset, sorting), checked against the WinForms menu. `FormStash` is on the list and the viewer. Step 4 done: the commit info (`CommitInfoViewModel`, `CommitInfoView` with `XhtmlTextBlock` for the XHTML of the WinForms renderers). `FormCommitDiff` composes the commit info, the list and the viewer. Step 5 done: the blame (`BlameViewModel`, `BlameView`, the author lines as a margin of the editor, with a parity test of the texts) with `FormBlame`, and `FormFileHistory` on the grid, the commit diff, the viewers and the blame. The commit info has its avatar and context menu. Step 6 done: `FormCommit` (staging, the message with its line limits, commit and push, amend, the commit kinds, the templates and the conventional messages, spell checking with `EditNetSpell` on AvaloniaEdit, the hotkeys, staging and resetting lines from the diff, GPG signing, the selection filter). `FormPush` is ported. `FormRemoteProcess` is ported: push, pull, fetch, clone and the other remote commands run git in the Avalonia progress dialog, with the questions of PuTTY. `FormResolveConflicts` is ported. Phase 5 is complete apart from the WinForms `FormPush`/`FormPull`/`FormClone`, which remain as fallbacks. `FormPull` is ported. `FormRebase` and `FormApplyPatch` are ported, on a port of `PatchGrid`. |
| 6: Settings dialog and plugin settings | **Done.** `FormSettings` with all its pages and the pages of the plugins (their settings through the value models of `SettingControlBindingsProvider`); used by default. See the [ledger](ledger.md). |
| 7: Plugin API v2 and switching the host | **In progress.** Host switch done for the Avalonia main window: Avalonia runs the message loop of the process (`AvaloniaUi.RunMainLoop`) until the last main window closes, and reports the exceptions of its jobs and of the background operations (`TaskManager.UnhandledExceptionHandler`) to the bug report. The `JoinableTaskContext` keeps the WinForms synchronization context while WinForms dialogs can still be opened from Avalonia code (an Avalonia context would deadlock their `SwitchToMainThreadAsync`); it moves to Avalonia's in phase 8. The forms of the plugins (and the repository hosting dialogs of the GitHub plugin) are ported on the current plugin API. The Avalonia main window (`BrowseWindow`, with `GE_AVALONIA=all,FormBrowse`): the revision grid with the commit and diff tabs, the main menus and toolbar. The grid has its context menu; the file tree tab, the filter toolbar, the dashboard and switching repositories in the window are ported. The console and GPG tabs and the left panel (`RepoObjectsTree`) are ported. The build report tab, the build status, avatar and notes columns, stashes, the plugins and scripts menus, the toolbar buttons and the hotkeys are ported. Next: the remaining gaps listed in the ledger. |
| 8+ | Not started. |
...
- **Plugin API v2, as implemented.** A plugin needs no WinForms type; `System.Drawing` (`Image`) stays until the cross-platform phase. The v1 members are unchanged, so v1 plugins load and run as before. Only `IRepositoryHostPlugin.ConfigureContextMenu` is `[Obsolete]`; the other v1 members point to their v2 replacement in their XML docs, because marking them would raise warnings in the WinForms fallback code.
  - **Owner window:** `WindowOwner(nint Handle)` (`None`) replaces `IWin32Window`. `GitUIEventArgs.Owner` gives it to the plugin. With the v1 constructor it is the handle of `OwnerForm`. The new `GitUIEventArgs(WindowOwner, ...)` constructor sets `OwnerForm` for v1 plugins: the WinForms form of the handle, or a wrapper of the handle for an Avalonia window. The Avalonia main window creates the arguments this way; `FormBrowse` keeps passing itself. `WindowOwnerWinFormsExtensions` (`ToWin32Window`, `ToWindowOwner`) is the bridge and goes away in phase 8.
  - **Message boxes:** `PluginMessageBoxes` (`Show`, `ShowError`, `ShowWarning`, `ShowInformation`, `Confirm`, and `ShowChoiceAsync` for task dialogs with custom buttons) uses its own enums (`PluginMessageBoxButtons`, `PluginMessageBoxIcon`, `PluginMessageBoxDefaultButton`, `PluginMessageBoxResult`). For now `WinFormsPluginMessageBoxService` shows them through `MessageBoxes` and `TaskDialog`; it is the one class to replace in phase 8.
  - **Commands:** `GitUICommandsExtensions` adds `WindowOwner` overloads of the `IGitUICommands` members the plugins use: `StartCommandLineProcessDialog` (×2), `StartGitCommandProcessDialog`, `StartSettingsDialog`, `StartPluginSettingsDialog`, `StartBrowseDialog` and `StartCommitDialog`. `AvaloniaPluginDialogs` gets `ShowDialog`/`Show` with a `WindowOwner` and `GetWindowOwner`.
  - **Menus:** `PluginMenuItem` (text, click action, icon, children, `IsEnabled`, `Separator`) is a menu model. A repository host plugin that implements `IBlameContextMenuProvider.GetBlameContextMenuItems(GitBlameContext)` gets its items added each time the blame context menu opens, in the WinForms `BlameControl` (`PluginMenuItemRenderer`) and in the Avalonia blame (`IBlameHost.GetRepositoryHostMenuItems`, shown with `MenuModelRenderer`). `ConfigureContextMenu` is then not called.
  - **Settings:** `ActionSetting` (a link with an action) replaces a `PseudoSetting` holding a WinForms `LinkLabel`. Its action gets a `SettingActionContext`: the owner, and the values being edited (`Values`, a `MemorySettingsSource` at the level of the page). The action can change those values, and both dialogs then show them.
  - **Build server settings:** a build server integration exports an `IBuildServerSettingsProvider` (`[BuildServerSettingsProviderMetadata(type)]`) instead of an `IBuildServerSettingsUserControl`. `GetSettings(BuildServerSettingsContext)` returns `ISetting`s, which both build server pages render as plugin settings (WinForms: `BuildServerSettingsProviderControl`; Avalonia: `BuildServerPluginSettings` shown by `PluginSettingsPageView`). The context has the default project name and the remote URLs. `SuggestValue` pre-fills an unset setting, and the suggestion is saved with the page, as the v1 controls did. The optional `Validate` blocks saving invalid values, as TeamCity's v1 control did. The host uses v2 when a plugin has both; v1 controls are still embedded when a plugin has only v1. `BuildServerSettings.GetSettingsSource` now carries the level of its settings.
  - **In-repo plugins:**
    - AutoCompileSubmodules, Gource, ProxySwitcher and GitHub (`args.Owner`, `PluginMessageBoxes`, `StartCommandLineProcessDialog(WindowOwner, ...)`) use v2.
    - The plugin dialogs (CreateLocalBranches, DeleteUnusedBranches, FindLargeFiles, Gource, ProxySwitcher, ReleaseNotesGenerator, Statistics) are shown over `args.Owner`.
    - GitHub also uses `ActionSetting` for its token links and `IBlameContextMenuProvider` (its `ConfigureContextMenu` is now an empty explicit implementation).
    - Azure DevOps's adapter uses `PluginMessageBoxes.ShowChoiceAsync` instead of `TaskDialog`.
    - AppVeyor, GitHub Actions, Jenkins and TeamCity (choose build, extract from the clipboard: `ActionSetting`s; invalid filter: `Validate`) use `IBuildServerSettingsProvider`. Their v1 controls stay, unused, until phase 8.
    - Azure DevOps and GitLab keep their v1 controls: they fetch data asynchronously from the server (the project ID, the build definition) and enable links as the text changes, which the setting kinds cannot express yet.
    - The WinForms fallback forms and their `ShowDialog(args.OwnerForm)` lines stay until phase 8.
