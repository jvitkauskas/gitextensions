...
///  the order of Windows; on macOS a row of buttons aligned to the right in a <c>Border.dialogFooter</c> is reversed, as a
///  row of buttons marked <c>StackPanel.dialogButtons</c> elsewhere (e.g. the footer of the settings, docked right).
...
internal static class DialogButtonOrder
{
    ...
    public static void Apply(ILogical root, bool reverse)
    {
        ...
        List<StackPanel> footerRows = [.. root.GetSelfAndLogicalDescendants()
            .OfType<Border>()
            .Where(border => border.Classes.Contains("dialogFooter"))
            .SelectMany(footer => footer.GetLogicalDescendants().OfType<StackPanel>())
            .Where(row => row.Orientation == Orientation.Horizontal && row.HorizontalAlignment == HorizontalAlignment.Right)];
        List<StackPanel> markedRows = [.. root.GetSelfAndLogicalDescendants().OfType<StackPanel>().Where(row => row.Classes.Contains("dialogButtons"))];
        List<StackPanel> rows = [.. footerRows.Union(markedRows)
            .Where(row => row.Children.Count > 1 && row.Children.All(child => child is Button and not ToggleButton))];
